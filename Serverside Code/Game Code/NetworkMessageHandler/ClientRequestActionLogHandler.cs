...
{
    class ClientRequestActionLogHandler : INetworkMessageHandler
    {
        public void HandleMessage(Player sender, Message message, ServiceContainer serviceContainer)
        {
            ...
            Message answer = Message.Create(NetworkMessageType.ServerSentActionLog.ToString("G"));
            answer = serviceContainer.GameRoomService.PlayerActionLog.DTO.ToMessage(answer);
